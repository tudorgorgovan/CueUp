...
Interogarile sunt scrise direct, cu valorile puse in text prin `_txt` / `_nr`,
care se ocupa de ghilimele si de conversia numerelor. Citirile care nu depind de
utilizator sunt puse in cache cu st.cache_data, iar scrierile golesc cache-ul.
"""
...
def _txt(valoare) -> str:
    """Scrie o valoare text asa cum o asteapta SQL Server: intre apostrofuri."""
    if valoare is None:
        return "NULL"
    return "'" + str(valoare).replace("'", "''") + "'"


def _nr(valoare) -> str:
    """Scrie o valoare numerica in interogare (int sau zecimal)."""
    if valoare is None:
        return "NULL"
    if isinstance(valoare, float):
        return repr(float(valoare))
...
def cauta_parteneri(id_oras: int, elo_min: int, elo_max: int,
    ...
    sql = f"""
        ...
        WHERE  j.id_oras   = {_nr(id_oras)}
          AND  j.rating_elo BETWEEN {_nr(elo_min)} AND {_nr(elo_max)}
          AND  j.id_jucator != {_nr(exclude_id)}
...
def mese_active_club(id_club: int, tip_masa: str = None) -> pd.DataFrame:
    if tip_masa:
        sql = f"""
            ...
            FROM   Mese
            WHERE  id_club = {_nr(id_club)} AND activa = 1
              AND  tip_masa = {_txt(tip_masa)}
...
def rezervari_jucator(id_jucator: int, status: str = None) -> pd.DataFrame:
    base = f"""
        ...
        WHERE  r.id_jucator = {_nr(id_jucator)}
    """
    if status:
        return _df(base + f" AND r.status = {_txt(status)} ORDER BY r.data_start DESC")
    return _df(base + " ORDER BY r.data_start DESC")
...
def orase_active(min_jucatori: int = 10) -> pd.DataFrame:
    ...
    sql = f"""
        ...
        HAVING COUNT(DISTINCT j.id_jucator) >= {_nr(min_jucatori)}
...
def clasament_jucatori_activi(min_rezervari: int = 3) -> pd.DataFrame:
    sql = f"""
        ...
        GROUP  BY j.id_jucator, j.nume, o.nume_oras,
                  j.rating_elo, j.nivel_declarat
        HAVING COUNT(r.id_rezervare) > {_nr(min_rezervari)}
        ORDER  BY j.rating_elo DESC
    """
    return _df(sql)
...
def get_jucator_by_id(id_jucator: int) -> pd.DataFrame:
    sql = f"""
        ...
        FROM   Jucatori j
        JOIN   Orase    o ON j.id_oras = o.id_oras
        WHERE  j.id_jucator = {_nr(id_jucator)}
    """
    return _df(sql)
...
def verify_login(email: str, parola: str):
    ...
    sql = f"""
        ...
        JOIN   Orase    o ON j.id_oras = o.id_oras
        WHERE  j.email                         = {_txt(email)}
          AND  REPLACE(j.parola, CHAR(13), '') = {_txt(parola)}
    """
    df = _df(sql)
...
def email_exists(email: str) -> bool:
    ...
    sql = f"SELECT COUNT(*) AS cnt FROM Jucatori WHERE email = {_txt(email)}"
    return int(_df(sql).iloc[0]["cnt"]) > 0
...
def register_player(nume: str, email: str, parola: str,
    ...
    sql = f"""
        ...
        VALUES ({_txt(nume)}, {_txt(email)}, {_nr(id_oras)}, {_nr(nivel_declarat)},
                1200, CAST(GETDATE() AS DATE), {_txt(parola)})
    """
    _executa(sql)
...
def get_rank_in_city(id_oras: int, rating_elo: int) -> int:
    sql = (f"SELECT COUNT(*) + 1 AS rank_oras FROM Jucatori "
           f"WHERE id_oras = {_nr(id_oras)} AND rating_elo > {_nr(rating_elo)}")
    return int(_df(sql).iloc[0]["rank_oras"])


@st.cache_data(ttl=_TTL_STATISTICI, show_spinner=False)
def get_rank_national(rating_elo: int) -> int:
    sql = (f"SELECT COUNT(*) + 1 AS rank_national FROM Jucatori "
           f"WHERE rating_elo > {_nr(rating_elo)}")
    return int(_df(sql).iloc[0]["rank_national"])


@st.cache_data(ttl=_TTL_STATISTICI, show_spinner=False)
def get_count_city(id_oras: int) -> int:
    sql = f"SELECT COUNT(*) AS total FROM Jucatori WHERE id_oras = {_nr(id_oras)}"
    return int(_df(sql).iloc[0]["total"])
...
def check_masa_disponibila(id_masa: int, data_start: str, data_sfarsit: str) -> bool:
    ...
    sql = f"""
        ...
        FROM   Rezervari
        WHERE  id_masa = {_nr(id_masa)}
          AND  status IN ('confirmata', 'finalizata')
          AND  data_start   < {_txt(data_sfarsit)}
          AND  data_sfarsit > {_txt(data_start)}
    """
    return int(_df(sql).iloc[0]["conflicte"]) == 0
...
def create_rezervare(id_masa: int, id_jucator: int,
    ...
    sql = f"""
        ...
        VALUES ({_nr(id_masa)}, {_nr(id_jucator)},
                {_txt(data_start)}, {_txt(data_sfarsit)},
                'confirmata', {_nr(float(suma))}, GETDATE())
    """
    _executa(sql)
...
def get_istoric_rating(id_jucator: int) -> pd.DataFrame:
    sql = f"""
        ...
        WHERE  ir.id_jucator = {_nr(id_jucator)}
...
def get_meciuri_jucator(id_jucator: int) -> pd.DataFrame:
    ...
    sql = f"""
        ...
        WHERE  (m.id_jucator_1 = {_nr(id_jucator)}
                OR m.id_jucator_2 = {_nr(id_jucator)})
